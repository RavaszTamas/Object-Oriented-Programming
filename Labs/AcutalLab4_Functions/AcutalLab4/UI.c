...
void UIFilterByMaxEnergyCapacity(int maxEnegryCapacity,DynamicVector* RobotRepository)
{
	...
	int numberOfFound = filterByMaxEnergyCapacity(maxEnegryCapacity,listofValidIndexes,RobotRepository,choice);
	int i = 0;
	char* outPutString = (char*)malloc(sizeof(char)*1000);
	for(i = 0; i < numberOfFound; i++)
	{
...
{
	int result = UndoFunctionService(RobotRepository,UndoRedoStackOfFunctionCalls);
	if(result == 0)//Failed undo
	{
		printf("%s","No more undo possibility!\n");
		return;
...
{
	int result = RedoFunctionService(RobotRepository, UndoRedoStackOfFunctionCalls);
	if (result == 0)//Failed redo
	{
		printf("%s", "No more redo possibility!\n");
		return;
	}
...
void UI()
{
	...
	while(1)
	{
		...
		{
			UIAddNewRobot(InputCommand+4,RobotRepository, UndoRedoStackOfFunctionCalls);
		}
		else if(strcmp(command,"update") == 0)
		{
			UIUpdateExistingRobot(InputCommand+7,RobotRepository, UndoRedoStackOfFunctionCalls);

		}
		else if(strcmp(command,"delete") == 0)
		{
			UIDeleteRobot(InputCommand+7,RobotRepository, UndoRedoStackOfFunctionCalls);
